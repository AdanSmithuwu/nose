...
public record EmpleadoCreateDto(
    ...
    @Size(max = DbConstraints.LEN_TELEFONO,
          message = "teléfono máximo " + DbConstraints.LEN_TELEFONO + " dígitos")
    @Pattern(regexp = "\\d{6," + DbConstraints.LEN_TELEFONO + "}",
             message = "teléfono debe tener entre 6 y " + DbConstraints.LEN_TELEFONO + " dígitos")
    String telefono,
    @NotNull(message = "idRol obligatorio")
    Integer idRol,
    /* Opcional. Si se omite se generará automáticamente */
