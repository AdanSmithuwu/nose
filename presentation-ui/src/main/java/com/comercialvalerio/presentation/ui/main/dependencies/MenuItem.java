...
public class MenuItem extends JPanel {
    ...
    private Icon getIcon() {
        ...
        FlatSVGIcon icon = new FlatSVGIcon(
                "com/comercialvalerio/presentation/ui/main/dependences/icon/" + menuIndex + ".svg");
        FlatSVGIcon.ColorFilter f = new FlatSVGIcon.ColorFilter();
        f.add(Color.decode(""+UIStyle.HEX_DISABLED+""), lightColor, darkColor);
    ...
    private void init() {
        ...
        for (int i = 0; i < menus.length; i++) {
            JButton menuItem = createButtonItem(menus[i], i);
                ...
                menuItem.addActionListener((ActionEvent e) -> {
                    if (menus.length > 1) {
                        if (menu.isMenuFull()) {
                            ...
                            popup.show(
                                    MenuItem.this,
                                    (int) MenuItem.this.getWidth() + UIScale.scale(5),
    ...
    private JButton createButtonItem(String text, int idx) {
        ...
        button.putClientProperty(FlatClientProperties.STYLE, ""
                ...
                + "margin:3,11,3,11");
        if (!text.isEmpty()) {
            MenuOption option = MenuOption.fromIndex(menuIndex);
            KeyStroke ks = MenuKeyMap.get(option);
            if (ks != null) {
                KeyUtils.setTooltipAndMnemonic(button, ks, text);
            } else {
                int key = KeyEvent.getExtendedKeyCodeForChar(Character.toUpperCase(text.charAt(0)));
                KeyUtils.setTooltipAndMnemonic(button, key, text);
            }
        }
        ...
        im.put(KeyStroke.getKeyStroke(KeyEvent.VK_ENTER, 0), "press");
        im.put(KeyStroke.getKeyStroke(KeyEvent.VK_ENTER, 0, true), "release");
        ...
        button.addFocusListener(new java.awt.event.FocusAdapter() {
            ...
            public void focusGained(java.awt.event.FocusEvent e) {
                menu.setSelected(menuIndex, idx);
            }
        });
